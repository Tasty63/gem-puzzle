const isMobile = /Mobile|IEMobile|mini|Android|iP(ad|od|hone)/i.test(navigator.userAgent);
const size3x3 = 9;
const size4x4 = 16;
const size5x5 = 25;
const size6x6 = 36;
...
export default class Field {
  ...
  moveTile(event) {
    ...
    const deltaX =
      Math.floor(target.getBoundingClientRect().left) - Math.floor(this.emptyTile.getBoundingClientRect().left);

    const deltaY =
      Math.floor(target.getBoundingClientRect().top) - Math.floor(this.emptyTile.getBoundingClientRect().top);

    if (
      (Math.abs(deltaX) === this.tileWidth && deltaY === 0) ||
    ...
    const content = `
            <div class='pop-up__content'>
                ...
                <div class='pop-up__time'>
                   Вы решили головоломку за <span class='time-counter'>
                   ${addZero(this.min)}:${addZero(this.sec)}</span> 
                </div> 
  ...
  isUnsolvable() {
    const orderOfTiles = this.arrTiles.slice().sort((value, nextValue) => value.style.order - nextValue.style.order);
    let numberOfPairs = 0;
